...
### Methods used in staff_profiles.py
    ...
    # return lists of all staffs and their details
    def get_all_staff():
        rows = db.__query('SELECT s.id, s.name, s.username, st.title FROM staff s, staff_type st WHERE s.staff_type_id = st.id AND st.id > %s', [0,])
    ...
    def get_staff_detail(staff_id):
        row = db.__query('SELECT s.id, s.name, s.username, s.staff_type_id FROM staff s WHERE s.id = %s', [staff_id,])
        print("ROW")
        print(row)
        if (not row):
            return None
    ...
    # modify staff's details based on the inputed parameter
    # return true if success and false otherwise
    def modify_staff(nid, nname, nusername, nstaff_type):
        return db.__update("UPDATE staff SET name = %s, username = %s, staff_type_id = %s WHERE id = %s", [nname, nusername, nstaff_type, nid])

    # delete a staff's record based on the inputed staff_id
    # return true if success and false otherwise
    def delete_staff(staff_id):
        return db.__delete("DELETE FROM staff WHERE id = %s", [staff_id,])